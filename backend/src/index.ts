...
app.use('*', cors({
  origin: (origin) => {
    const allowed = [
      ...
      'https://nexasistem-app.pages.dev',
      'https://nexasistem-admin.pages.dev',
      'https://nexasistem-web.pages.dev',
      'http://localhost:3000',
      'http://localhost:3001',
      'http://localhost:3002',
    ]
    // Allow semua subdomain pages.dev untuk preview deploys
    if (origin && (origin.endsWith('.pages.dev') || origin.endsWith('.workers.dev'))) {
      return origin
    }
