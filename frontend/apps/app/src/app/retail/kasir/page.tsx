...
export default function RetailKasirPage() {
  ...
  useEffect(() => {
    apiFetch(`/api/products?module=retail&limit=100${q ? '&q=' + q : ''}`).then(d => setProducts(d.products || [])).finally(() => setLoading(false))
  }, [q])
  ...
  async function handlePay() {
    ...
    try {
      const data = await apiFetch('/api/orders', {
        method: 'POST',
        body: JSON.stringify({ module: 'retail', items: cart.map(i => ({ product_id: i.id, name: i.name, qty: i.qty, unit: i.unit || 'pcs', price: i.price, subtotal: i.price * i.qty })), paid_amount: method === 'cash' ? parseInt(paid) : subtotal, payment_method: method }),
      })
  ...
  return (
    <AppLayout title="Kasir Retail" subtitle="Point of Sale">
    <div style={{display:'flex',gap:16,height:'calc(100vh - 60px - 48px)'}}>
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="bg-white border-b border-gray-100 p-4">
          <h1 className="font-bold text-gray-900 mb-3">Kasir Retail</h1>
          <input className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
            placeholder="Cari produk / barcode..." value={q} onChange={e => setQ(e.target.value)} />
        </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {products.map((p: any) => (
                <button key={p.id} onClick={() => addToCart(p)} disabled={p.stock <= 0}
                  className="bg-white rounded-xl border border-gray-100 p-4 text-left hover:border-indigo-300 hover:shadow-md transition-all disabled:opacity-50 active:scale-95">
                  ...
                  <div className={`text-xs mt-0.5 ${p.stock <= p.min_stock ? 'text-red-500' : 'text-gray-400'}`}>Stok: {p.stock}</div>
                </button>
              ))}
            </div>
          )}
      ...
      <div className="w-72 bg-white border-l border-gray-100 flex flex-col">
        <div className="p-4 border-b border-gray-100 font-semibold">Keranjang</div>
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {cart.length === 0 ? <div className="text-center py-8 text-gray-400 text-sm">Pilih produk</div> : cart.map((item: any) => (
        ...
        {cart.length > 0 && (
          <div className="p-4 border-t border-gray-100 space-y-3">
            <div className="flex justify-between font-bold text-lg"><span>Total</span><span className="text-indigo-600">{formatRp(subtotal)}</span></div>
            ...
            <button onClick={handlePay} disabled={paying} className="w-full py-3 bg-indigo-600 text-white rounded-lg font-semibold disabled:opacity-50">{paying ? 'Memproses...' : 'Bayar'}</button>
          </div>
        )}
      </div>
    </div>
