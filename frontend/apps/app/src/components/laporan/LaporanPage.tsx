...
export default function LaporanPage({{ module, title }}: {{ module: string; title: string }}) {{
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 flex flex-wrap gap-3 items-center">
          ...
          <div className="flex items-center gap-2">
            <input type="date" className="px-3 py-1.5 border border-gray-200 rounded-lg text-sm" value={{from}} onChange={{e => setFrom(e.target.value)}} />
            <span className="text-gray-400">—</span>
            <input type="date" className="px-3 py-1.5 border border-gray-200 rounded-lg text-sm" value={{to}} onChange={{e => setTo(e.target.value)}} />
          </div>
        </div>

        {{loading ? <div className="text-center py-12 text-gray-400">Memuat...</div> : data && (
          <>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {{[
                {{ label: 'Total Transaksi', value: data.summary?.total_transactions ?? 0, fmt: (v: number) => v.toString() }},
                {{ label: 'Gross Revenue', value: data.summary?.gross_revenue ?? 0, fmt: formatRp }},
                {{ label: 'Total Diskon', value: data.summary?.total_discount ?? 0, fmt: formatRp }},
                {{ label: 'Net Revenue', value: data.summary?.net_revenue ?? 0, fmt: formatRp }},
              ].map(s => (
